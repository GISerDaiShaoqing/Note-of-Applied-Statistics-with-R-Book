...
检验两个变量之间是否存在线性相关关系，等价于对回归系数$$\beta_1$$的检验。采用R. A. Fisher提出的t检验。
检验的步骤为：
...
$$ E(y)=\beta_0+\beta_1x $$
对于所有的x值，ε的方差$$\sigma^2$$都相同；误差项ε是一个服从正态分布的随机变量，且相互独立。即ε~N(0,$$\sigma^2$$);独立性意味着对于一个特定的x值，它所对应的ε与其他x值所对应的ε不相关；对于一个特定的x值， 它所对应的y值与其他x所对应的y值也不相关。
...
总体回归参数$$\beta_0$$和$$\beta_1$$是未知的，必须利用样本数据去估计。用样本统计量$$b_0$$和$$b_1$$代替回归方程中的未知参数$$\beta_0$$和$$\beta_1$$，就得到了估计的回归方程。
简单线性回归中估计的回归方程为
...
> * 确定显著性水平$$\alpha$$，并进行决策：
$$ \left|t\right|\gt t_{\alpha/2}$$,拒绝$H_0$;$$\left|t\right|\lt t_{\alpha/2}$$，不拒绝$$H_0 $$
...
> * 误差项ε是一个服从正态分布的随机变量，即$$\varepsilon~N(0,\sigma^2)$$，且相互独立。
...
> （1）提出假设
$$H_0:\beta_i=0)$$(自变量$$x_i$$与因变量y没有线性关系
...
没有交互项, $$x_1$$对y的影响用$$\beta_1$$测量；有交互项,$$x_1$$对y的影响用$$\beta_1+\beta_3x_2$$测量。影响随$$x_2$$的改变而改变。
...
个数的增加而增加。最小的MSE可作为最优变量选择的一个准则，但需考虑所有子集 ($$2^p$$个)。
